...
var leftArrow = document.querySelector('.slider__arrow--left'),
    ...
    // Size of the slider
canMove = true,
    // Boolean blocking the navigation during transitions
...
var slidesLength = slides.querySelectorAll('.slider__slide').length; // Go to next slide when clicking right arrow

rightArrow.addEventListener('click', next); // Go to previous slide when clicking left arrow
...
function next() {
  if (canMove && position < slidesLength - 1) {
    moveTo(position + 1); // If the end is reached after the move
...
function moveTo(newPosition, jump) {
  if (!jump) {
    // Block the navigation for the duration of the transition (except when juming)
    canMove = false;
    setTimeout(function () {
      canMove = true;
    }, 300);
